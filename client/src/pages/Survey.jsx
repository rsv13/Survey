...
const SurveyForm = () => {
  ...
  const handleSurveySubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/survey/surveyQuestion`, {
        method: "POST",
      ...
      <div className="max-w-4xl w-full p-8 bg-white dark:bg-gray-700 shadow-lg rounded-lg">
        {!showSurvey ? (
          <form className="space-y-6" onSubmit={handleSubmit}>
            <h1 className="text-3xl font-semibold text-center text-gray-800 dark:text-gray-200 mb-6">Survey Registration</h1>
            <p className="text-lg text-gray-700 dark:text-gray-300 mb-6 text-center">
              Welcome! We appreciate your participation in our survey. Your responses will help us better understand community needs and improve our services. Please provide your information and
              consent to proceed to the survey questions.
            </p>
            <div>
              <Label htmlFor="surveyUsername" value="Survey Username" />
              <TextInput id="surveyUsername" value={formData.surveyUsername} readOnly disabled />
            </div>
            <div>
            ...
            <div>
              ...
              <TextInput id="designation" value={formData.designation} onChange={handleFormChange} placeholder="Enter your designation" />
            </div>
            <div>
          ...
          <form className="space-y-6" onSubmit={handleSurveySubmit}>
            <h1 className="text-3xl font-semibold text-center text-gray-800 dark:text-gray-200 mb-6">Survey Questions</h1>
                ...
                <div className="flex flex-wrap gap-4">
                  {surveyInputs.options.map((option, i) => (
                    <div key={i} className="flex items-center">
                      <Radio id={`q${index}-opt${i}`} name={`q${index}`} value={i + 1} checked={surveyAnswers[index] === i + 1} onChange={() => handleSurveyChange(index, i + 1)} />
        ...
        {submitted && !error && (
          <div className="text-center">
            <p className="text-green-600 dark:text-green-400 text-lg mt-6">Thank you for completing the survey!</p>
            <Button gradientDuoTone="purpleToPink" className="mt-4" onClick={handleViewResults}>
              Check Results
            </Button>
          </div>
        )}

...
export default function DashSurveys() {
  ...
  useEffect(() => {
    ...
    const filtered = userSurveys.filter(
      (survey) =>
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setUserSurveys((prevSurveys) => prevSurveys.filter((survey) => survey._id !== selectedSurveyId));
        setFilteredSurveys((prevFilteredSurveys) => prevFilteredSurveys.filter((survey) => survey._id !== selectedSurveyId));
      }
    } catch (error) {
  ...
  return (
    <div className="p-3">
      <div className="flex items-center mb-4">
        <TextInput type="text" placeholder="Search surveys..." rightIcon={AiOutlineSearch} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full max-w-md" />
      </div>
            ...
            <Table.Body className="divide-y">
              {filteredSurveys.map((survey) => (
                <Table.Row key={survey._id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                  ...
                  <Table.Cell>{[survey.country || "N/A", survey.state || "N/A", survey.city || "N/A"].join(", ")}</Table.Cell>
                  <Table.Cell>
                    <Button gradientMonochrome="info" onClick={() => handleViewClick(survey)}>
                      View
                    </Button>
                  </Table.Cell>
                  {currentUser?.isAdmin && (
                    <Table.Cell>
                      <Button onClick={() => handleDeleteClick(survey._id)} color="failure">
                        Delete
                      </Button>
                    </Table.Cell>
                  )}
      ...
      <Modal show={showModal} onClose={() => setShowModal(false)} popup size="md">
        ...
        <Modal.Body>
          <div className="text-center">
            ...
            <h3 className="mb-5 text-lg text-gray-500 dark:text-gray-400">Are you sure you want to delete this survey?</h3>

...
export default function DashUsers() {
  ...
  const handleAssignRole = async () => {
    try {
      ...
      if (res.ok) {
        setUsers((prev) => prev.map((user) => (user._id === userIdToEdit ? { ...user, role: newRole } : user)));
  ...
  return (
    <div className="table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500">
            ...
            <Table.Body className="divide-y">
              {users.map((user) => (
                <Table.Row key={user._id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                  ...
                  <Table.Cell className={user.role === "Admin" ? "text-red-500" : user.role === "Group Admin" ? "text-teal-500" : user.role === "normalUser" ? "text-blue-500" : "text-gray-500"}>
                    {user.role === "normalUser" ? "User" : user.role}
                  </Table.Cell>
                  <Table.Cell colSpan={2} className="flex justify-center gap-4">
                    <span onClick={() => handleOpenRoleModal(user._id, user.role)} className="font-medium text-green-500 hover:underline cursor-pointer">
                      Change Role
                    </span>
                    <span
                      onClick={() => {
      ...
      <Modal show={showRoleModal} onClose={() => setShowRoleModal(false)} popup size="md">
        ...
        <Modal.Body>
          <div className="text-center">
            <h3 className="mb-5 text-lg text-gray-500 dark:text-gray-400">Assign a new role to the user</h3>
            <Select value={newRole} onChange={(e) => setNewRole(e.target.value)} className="mb-4">
              <option value="normalUser">User</option>
              <option value="Group Admin">Group Admin</option>
              <option value="Admin">Admin</option>
            </Select>
            {newRole === "Group Admin" && (
              <Select value={selectedGroup} onChange={(e) => setSelectedGroup(e.target.value)} className="mb-4">

...
export default function DashGroupUsers() {
  ...
  useEffect(() => {
    if (searchQuery) {
      setFilteredUsers(users.filter((user) => user.surveyUsername.toLowerCase().includes(searchQuery.toLowerCase())));
    } else {
  ...
  return (
    <div className="p-3">
      <div className="flex items-center mb-4">
        <TextInput type="text" placeholder="Search users..." rightIcon={AiOutlineSearch} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full max-w-md" />
      </div>
